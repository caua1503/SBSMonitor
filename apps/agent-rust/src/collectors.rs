//! Win32-only collectors. The selected APIs are present on Windows XP.
...
pub fn collect_cpu(sample: Duration) -> Result<CpuMetrics, AgentError> {
    ...
    let used = if total_delta == 0 {
        0.0
    } else {
        (1.0 - idle_end.saturating_sub(idle_start) as f64 / total_delta as f64) * 100.0
    };
    Ok(CpuMetrics {
        used_percentage: round_two(used),
    })
}
